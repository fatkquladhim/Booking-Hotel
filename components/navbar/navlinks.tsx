        ...
        <>
            {session ?.user ? (
                <div className="flex items-center justify-end md:order-2 gap-3">
                    <div className="hidden text-sm border rounded-full bg-gray-50 md:me-0 md:block focus:ring-4 focus:ring-gray-300">
                       <Image src={session.user.image || "/avatar.jpg"} alt='avatar' width={64} height={64} className="size-8 rounded-full" />
                    </div>
                    <div>
                        <button onClick={() => signOut()} className='md:block hidden py-2 px-4 bg-gray-50 text-gray-800 hover:bg-gray-100 rounded-sm cursor-pointer'>
                            Sign Out
                        </button>
                    </div>
                </div>
            ...
            <div className={clsx("w-full md:w-auto md:block", { "hidden": !open })}>
