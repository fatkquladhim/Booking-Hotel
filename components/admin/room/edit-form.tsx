...
import { UdateRoom } from "@/lib/actions";
...
const deleteimage = (image: string)=> {
    ...
    const [state, fromAction, isPending] = useActionState(UdateRoom.bind(null,image,room.id), null)
    ...
    <form action={fromAction}>
        <div className="grid md:grid-cols-12 gap-5 ">
            <div className="col-span-8 bg-white p-4 rounded-md shadow">
                <div className="mb-4">
                    <input type="text" name="name" defaultValue={room.name} className="py-2 px-4 border border-gray-400 rounded-sm w-full" placeholder="Room Name..."/>
                    <div aria-live="polite"  aria-atomic="true">
                        <span className="text-sm text-red-500 mt-2">{state?.error?.name}</span>
                    </div>
                </div>
                <div className="mb-4">
                    <textarea name="description" defaultValue={room.description} className="py-2 px-4 border border-gray-400 rounded-sm w-full " placeholder="Description" rows={8}></textarea>
                    <div aria-live="polite"  aria-atomic="true">
            ...
            <div className="col-span-4 bg-white p-4 rounded-md shadow">
                ...
                <div className="mb-4">
                    <input type="text" name="capacity" defaultValue={room.capacity} className="py-2 px-4 border border-gray-400 rounded-sm w-full" placeholder="Capacity..."/>
                    <div aria-live="polite"  aria-atomic="true">
                        <span className="text-sm text-red-500 mt-2">{state?.error?.capacity}</span>
                    </div>
                </div>
                <div className="mb-4">
                    <input type="text" name="price" defaultValue={room.price} className="py-2 px-4 border border-gray-400 rounded-sm w-full" placeholder="Price..."/>
                    <div aria-live="polite"  aria-atomic="true">
